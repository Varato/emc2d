...
model = np.ones(shape=(149, 149), dtype=np.float)#build_model(img, model_size=(149, 149), mean_count=0.2)
...
print("diff:", np.max(np.abs(model - model_recon)))

fig, (ax1, ax2, ax3) = plt.subplots(ncols=3)
ax1.imshow(model)
ax2.imshow(model_recon)
