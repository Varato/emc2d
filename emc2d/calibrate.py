...
def calibrate_drifts_with_reference(
    ...
    centre_drift_index = drift_radius[1] + drift_radius[0] * (2 * drift_radius[1] + 1)
    ...
    calibrating_shift = np.array(drift_radius) - recon_centre_drift
    return calibrating_shift


def centre_by_first_frame(frame_positions, drift_radius: Tuple[int, int], centre_is_origin=True):
    first_frame_position = frame_positions[0]
    calibrating_shift = np.array(drift_radius) - first_frame_position
    frame_positions += calibrating_shift
    if centre_is_origin:
        frame_positions -= np.array(drift_radius)
